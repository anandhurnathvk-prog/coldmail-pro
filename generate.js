export default async function handler(req, res) {
  ...
  const GOAL_MAP = {
    ...
    reply: "simply get a reply and start a conversation",
    partnership: "explore a potential partnership",
  };
  const TONE_MAP = {
    direct: "direct and confident — no fluff, get to the point",
  ...
  const prompt = `You are a world-class B2B sales copywriter. Your emails are famous for sounding completely human — warm, specific, and never robotic. Real people read them and think a real thoughtful human wrote them.
...
- NEVER use buzzwords: synergy, leverage, circle back, touch base, reach out, game-changer, holistic
...
Tips = 3 specific suggestions to improve reply rate for THIS specific email.`;
  ...
  try {
    const geminiRes = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${apiKey}`,
      {
        ...
        body: JSON.stringify({
          ...
          generationConfig: { temperature: 0.92, topP: 0.95, maxOutputTokens: 1200 },
        }),
      }
    );
    ...
    if (!geminiRes.ok) {
      ...
      return res.status(502).json({ error: errData?.error?.message || `Gemini error ${geminiRes.status}` });
    }

    const geminiData = await geminiRes.json();
    const raw = geminiData?.candidates?.[0]?.content?.parts?.[0]?.text || "";
    if (!raw) return res.status(502).json({ error: "Empty response from AI. Please try again." });
    ...
    try {
      ...
      if (!match) return res.status(502).json({ error: "Could not parse AI response. Please try again." });
      try { parsed = JSON.parse(match[0]); }
      catch { return res.status(502).json({ error: "Malformed AI response. Please try again." }); }
    }

    if (!parsed.subject || !parsed.body) return res.status(502).json({ error: "Incomplete AI response. Please try again." });
